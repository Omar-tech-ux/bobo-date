...
import { accountRoute, getSafeReturnRoute } from "../navigation";
import { activities } from "../types";
import { NotificationSetup } from "./NotificationSetup";
import { clearPendingInvitationBadge } from "./pushNotifications";
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (mode === "signin") {
        ...
        await refresh();
        goTo(getSafeReturnRoute());
      } else {
        const cleanUsername = username.trim().toLowerCase();
        if (!/^[a-z0-9_]{3,20}$/.test(cleanUsername)) {
          throw new Error(
            "Your tiny username needs 3–20 letters, numbers, or underscores.",
          );
        }
        const { data, error } = await supabase.auth.signUp({
          ...
          options: {
            ...
            emailRedirectTo: getAppUrl(accountRoute(getSafeReturnRoute("/pair"))),
          },
        });
        if (error) throw error;
        if (data.session) {
          await refresh();
          goTo(getSafeReturnRoute("/pair"));
        } else {
          setMessage(
            "A confirmation letter is waiting in your email ♡ Open it, then come back here to sign in.",
          );
        }
      }
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
          : "The mailbox got shy. Please try again.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <section className='love-auth'>
      ...
      <form onSubmit={submit} className='love-auth-form'>
        ...
        {message && (
          <p className='mailbox-message' role='status'>
            {message}
          </p>
        )}
        <button
          className='pixel-button mailbox-submit'
          data-sound='confirm'
          disabled={busy}
        >
          {busy
            ? "SENDING A TINY LETTER…"
            : mode === "signin"
              ? "OPEN OUR MAILBOX ♥"
              : "CREATE MY ACCOUNT ♥"}
        </button>
...
export function AccountPage() {
  ...
  return (
    <main className='online-screen'>
      <div className='mailbox-sky' aria-hidden='true'>
        <i>♥</i>
        <i>✦</i>
        <i>♥</i>
      </div>
      <section className='mailbox-paper'>
        <div className='mailbox-mark' aria-hidden='true'>
          ✉
        </div>
        <h1>Our little love mailbox</h1>
        <p className='mailbox-intro'>Only two people belong here: you and I.</p>
        ...
        {loading ? (
          ...
          <div className='account-home'>
            <p className='hello-person'>
              Hi, {profile?.display_name ?? "cutie"} ♡
            </p>
            ...
            {pairing.partner ? (
              <div className='paired-note'>
                ...
                <p>
                  ...
                  <strong>{pairing.partner.display_name}</strong> (@
                  {pairing.partner.username}).
                </p>
              </div>
            ) : (
              <div className='unpaired-note'>
                <p>Your person hasn’t joined this mailbox yet.</p>
                <a href='#/pair' className='pixel-button link-button'>
                  PAIR OUR ACCOUNTS
                </a>
              </div>
            )}
            ...
            <div className='account-actions'>
              <a href='#/inbox' className='pixel-button link-button'>
                SEE OUR INVITATIONS
              </a>
              <button
                type='button'
                className='text-link signout-link'
        ...
        <a className='mailbox-home-link' href='#/'>
          ← back to our pink sky
        </a>
      </section>
    </main>
  );
  ...
  const makeCode = async () => {
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "The code got lost in the clouds.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const join = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "That code did not find its person.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <main className='online-screen pairing-screen'>
      <section className='mailbox-paper pairing-paper'>
        <div className='pair-cats' aria-hidden='true'>
          /ᐠ｡ꞈ｡ᐟ\ &nbsp; ♥ &nbsp; /ᐠ｡ꞈ｡ᐟ\
        </div>
        ...
        {pairing.partner ? (
          <div className='paired-complete'>
            <p>
              {profile?.display_name} + {pairing.partner.display_name}
            </p>
            <strong>Two accounts, one tiny world ♡</strong>
            <a href='#/inbox' className='pixel-button link-button'>
              OPEN OUR INVITATIONS
            </a>
          </div>
        ) : (
        ...
        {message && (
          <p className='mailbox-message' role='alert'>
            {message}
          </p>
        )}
        <a href='#/account' className='mailbox-home-link'>
          ← back to my account
        </a>
      </section>
    </main>
  ...
  return (
    <a className='invitation-row' href={`#/invite/${invitation.id}`}>
      ...
      <span className='invitation-main'>
        ...
        <span>
          {sent ? "You sent this" : "Your person invited you"} ·{" "}
          {preview?.guest ?? invitation.date}
        </span>
      </span>
      <span className={`invite-status invite-status--${invitation.status}`}>
...
export function InboxPage() {
  const { configured, loading, user, pairing, receivedInvites, sentInvites } =
    useOnline();

  useEffect(() => {
    if (user) void clearPendingInvitationBadge()
        ...
        {!pairing.partner ? (
          ...
          <>
            <div className='inbox-toolbar'>
              <p>
                {pairing.partner.display_name} is your one and only date
                recipient ♡
              </p>
              <a href='#/plan' className='pixel-button link-button'>
                INVITE THEM
              </a>
            </div>
            <NotificationSetup userId={user!.id} compact />
            <section className='invite-list'>
              ...
              {receivedInvites.length ? (
                ...
                <p className='empty-list'>
                  No incoming tickets yet. The mailbox is listening.
                </p>
              )}
            </section>
            <section className='invite-list'>
              ...
              {sentInvites.length ? (
                sentInvites.map((invite) => (
                  <InvitationRow key={invite.id} invitation={invite} sent />
                ))
              ) : (
                <p className='empty-list'>
                  You haven’t sent a date ticket yet.
                </p>
              )}
            </section>
          </>
...
export function InvitationPage({
  ...
  const invitation = useMemo(
    () =>
      [...receivedInvites, ...sentInvites].find(
    ...
    return (
      <main className='online-screen'>
        <section className='mailbox-paper missing-invite'>
          ...
          <p>
            It may belong to the other cutie, or it is still flying through the
            clouds.
          </p>
          <a href='#/inbox' className='pixel-button link-button'>
            BACK TO OUR MAILBOX
  ...
  const respond = async (status: "accepted" | "needs_changes" | "declined") => {
    ...
    setMessage("");
    try {
      const notification = await respondToInvitation(invitation.id, status, note);
      const replyMessage = status === "accepted"
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "The reply got tangled in ribbon.",
      );
    } finally {
      setBusy(false);
    }
      ...
      <section className='invite-ticket-online'>
        <div className='online-ticket-heading'>
          ...
          <div>
            ...
            <p>
              {incoming
                ? `${pairing.partner?.display_name ?? "Your person"} invited you`
                : `sent to ${pairing.partner?.display_name ?? "your person"}`}
            </p>
          </div>
        </div>
        <div className='online-ticket-times'>
          <div>
            <span>Their chosen time</span>
        ...
        <p
          className={`big-invite-status big-invite-status--${invitation.status}`}
        >
          {statusCopy[invitation.status]}
        </p>
        {deliveryWarning && (
        ...
        {invitation.response_note && (
          <blockquote>“{invitation.response_note}”</blockquote>
        )}
        ...
        {incoming && invitation.status === "pending" && (
          <div className='response-box'>
            ...
            <div className='response-actions'>
              <button
                className='pixel-button accept-invite'
                disabled={busy}
              ...
              <button
                className='pixel-button reschedule-invite'
                disabled={busy}
                onClick={() => void respond("needs_changes")}
              >
                SUGGEST ANOTHER TIME
              </button>
              <button
                className='text-link decline-invite'
                disabled={busy}
        ...
        <a href='#/inbox' className='mailbox-home-link'>
          ← all invitations
        </a>
      </section>
    </main>
  ...
  return (
    <main className='online-screen'>
      <section className='mailbox-paper missing-invite'>
        ...
        <a href='#/' className='pixel-button link-button'>
          BACK TO THE PINK SKY
        </a>
      </section>
    </main>
  );
